...
fn test_put_object() {
    ...
    let url = ValidS3Url::parse("https://jsonlog.s3.amazonaws.com/test/test2.json").unwrap();
    let content = "{\"c\":\"d\"}\n".as_bytes().to_vec();
    let sha = s3_headers_lite::aws_math::get_sha256(&content);
    ...
    let (status_code, _, body) = assert::request_put(url.as_url().clone(), headers, content);
